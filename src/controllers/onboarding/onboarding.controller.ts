...
export const onboardingController = {
  ...
  selectDietType: async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const user = await User.findByIdAndUpdate(
        ...
        { dietTypes },
        { new: true }
      ).populate("dietTypes", "_id name img").lean();
  ...
  //     const user = await User.findByIdAndUpdate(userId, { dietTypes }, { new: true })
  //       .populate("dietTypes", "name img");
  ...
  //   try {
  //     const dietTypes = await DietTypeModel.find().select("name img");
  ...
  getAllDietType: async (req: Request, res: Response, next: NextFunction) => {
    try {
      const dietTypes = await DietTypeModel
        .find({}, { _id: 1, name: 1, img: 1 })
        .lean();
  ...
  //     const validAllergies = await FoodAllergyModel.find({ _id: { $in: allergies } }).select("_id name");
  //     if (validAllergies.length !== allergies.length) {
  //       return httpResponse(req, res, 404, "Some allergies not found");
  //     }

  //     const user = await User.findByIdAndUpdate(userId, { allergies }, { new: true })
  //       .populate("allergies", "name img");

  //     if (!user) return httpResponse(req, res, 404, "User not found");

  //     return httpResponse(req, res, 200, "Allergies updated successfully", { user });
  //   } catch (error) {
  //     return httpError(next, error, req, 500);
  //   }
  // },
  ...
  //   try {
  //     const allergies = await FoodAllergyModel.find().select("name img");
  ...
  selectAllergies: async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const validAllergies = await FoodAllergyModel.find({ _id: { $in: allergies } }).select("_id name").lean();
      ...
      const user = await User.findByIdAndUpdate(
        ...
        { allergies },
        { new: true }
      ).populate("allergies", "_id name img").lean();
  ...
  getAllAllergies: async (req: Request, res: Response, next: NextFunction) => {
    try {
      const allergies = await FoodAllergyModel
        .find({}, { _id: 1, name: 1, img: 1 })
        .lean();
  ...
  // moodGoals: async (req: Request, res: Response, next: NextFunction) => {
  //   try {
  //     const userId = req.user?.userId;
  //     const { moodGoalId } = req.params; // Using moodGoalId instead of moodGoal
  ...
  //     const user = await User.findByIdAndUpdate(userId, { moodGoal: moodGoalId }, { new: true }).select("moodGoal");

  //     return user 
  //       ? httpResponse(req, res, 200, "Mood goal updated successfully", { moodGoal: user.moodGoal }) 
  ...
  // GetAllmoodGoals: async (req: Request, res: Response, next: NextFunction) => {
  //   try {
  //     const userId = req.user?.userId;
  //     if (!userId) return httpResponse(req, res, 400, "User ID is required");

  //     // Fetch user data with mood goals and likes
  ...
  //       })
  //       .select("moodGoal likes -_id")
  //       .lean();

  //     // Fetch all mood goals
  //     const allMoods = await MoodGoalModel.find().select("_id name emoji");

  //     if (!allMoods) return httpResponse(req, res, 404, "Moods not found");

  //     // Return both mood goals and user's mood goal
  ...
  profileUpdate: async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const user = await User.findByIdAndUpdate(
        ...
        { $set: updateFields },
        { new: true }
      ).select("gender age weight height").lean();
  ...
  //     const user = await User.findByIdAndUpdate(
  //       userId,
  //       { activityLevel: activityLevelid, onboardingCompleted: true },
  //       { new: true }
  //     ).select("_id activityLevel onboardingCompleted");
  ...
  activityLevel: async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const user = await User.findByIdAndUpdate(
        ...
        { activityLevel: activityLevelid, onboardingCompleted: true },
        { new: true }
      ).select("_id activityLevel onboardingCompleted gender age weight height userName userEmail userProfile friends dietTypes allergies saved verified notifications likes createdAt updatedAt").lean();
  ...
  getAllactivityLevel: async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const activityLevels = await ActivityLevelModel
        .find({}, { level: 1, description: 1, _id: 1 }) // efficient projection
