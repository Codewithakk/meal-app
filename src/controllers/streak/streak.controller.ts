...
import { IMoodGoal, StreakMood } from '../../models/streakMood.model'
import { io } from '../../app'
...
export const moodGoals = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(userId, { moodGoal: moodGoalId }, { new: true }).select('moodGoal').lean()
...
export const GetAllmoodGoals = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const allMoods = await MoodGoalModel.find({}, { _id: 1, name: 1, emoji: 1 }).lean()
                ...
                $addFields: {
                    rankScore: {
                        $cond: [
                            { $gt: ['$maxMoodCount', 0] },
                            {
                                $multiply: [{ $divide: ['$moodCount', '$maxMoodCount'] }, 100]
                            },
                            0
                        ]
                    }
