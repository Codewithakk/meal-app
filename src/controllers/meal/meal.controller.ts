...
export const mealsController = {
    ...
    manageSavedMeals: async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            return httpResponse(req, res, 200, isSaved ? 'Meal saved successfully' : 'Meal removed from saved list', {
    ...
    groceryList: async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            const details = await recipeModel.findById(recipeid).select('ingredients').populate('ingredients').lean()
