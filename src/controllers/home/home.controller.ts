...
// import { MoodGoalModel, MoodGoal } from '../../models/moodGoal';
...
// // ✅ **Helper function to find user by ID (Lean mode)**
// const getUserById = async (userId: string, fields: string) =>
//   await User.findById(userId).select(fields).lean();

// export const homeController = {
//   shareRecipe: async (req: Request, res: Response, next: NextFunction): Promise<void> => {
//     try {
//       const { recipeId } = req.params;

//       if (!recipeId) {
//         return httpResponse(req, res, 400, "Recipe ID is required");
//       }

//       // Get the correct base URL
//       const baseUrl = getBaseUrl();

//       // Generate the properly formatted shareable URL
//       const shareableLink = `${baseUrl}/api/v1/home/recipeDetails?recipeid=${recipeId}`;

//       return httpResponse(req, res, 200, "Recipe shareable link generated", { shareableLink });
//     } catch (error) {
...
//   // **Fetch Hero Data & Top Meals**
//   heroAndTopMealOfDay: async (req: Request, res: Response, next: NextFunction): Promise<void> => {
//     try {
...
//       // Fetch user with moodGoal and likes
//       const [user, topRatedMeals] = await Promise.all([
...
//       // Modify topRatedMeals to include `isFavorite` flag
//       const updatedTopRatedMeals = topRatedMeals.map((meal) => ({
//         ...meal,
...
//           moodGoal: user.moodGoal || null, // Ensure moodGoal is handled properly
//           // likes: user.likes?.map((like) => ({
//           //   isFavorite: like.isFavorite,
//           //   recipeId: like.recipe,
//           // })) || [],
//         },
//         TopRatedMeals: updatedTopRatedMeals,
//       });
//     } catch (error) {
...
//       // Fetch mood goals and populate the 'moodGoal' field
...
//       return httpResponse(req, res, 200, "Mood goals retrieved successfully", { moods: user });
//     } catch (error) {
...
//       // Fetch all meals sorted by rating
//       const mealsOfDay = await Recipe.aggregate([
//         { $sort: { rating: -1 } }, // Sort by highest rating
//         {
//           $project: {
//             _id: 1,
...
//       if (!mealsOfDay || mealsOfDay.length === 0) {
//         return httpResponse(req, res, 404, "No meals found");
//       }
  
//       let updatedMealsOfDay;
//       if (userId) {
//         // Fetch user likes if authenticated
//         const user = await User.findById(userId).select("likes").lean();
//         const likedRecipes = new Set(user?.likes?.map((like) => like.recipe.toString()) || []);
  
//         // Map meals and set isFavorite based on likedRecipes
//         updatedMealsOfDay = mealsOfDay.map((meal) => ({
//           ...meal,
//           isFavorite: likedRecipes.has(meal._id.toString()), // True if liked, false otherwise
//         }));
//       } else {
//         // If no user, set isFavorite to false for all meals
//         updatedMealsOfDay = mealsOfDay.map((meal) => ({
//           ...meal,
//           isFavorite: false,
//         }));
...
//         mealsOfDay: updatedMealsOfDay, // Changed key to plural to reflect multiple meals
//       });
//     } catch (error) {
//       next(httpError(next, error, req, 500));
//     }
...
//         .select(
//           "imageUrl mood name rating description prepTime calories protein ingredients instructions benefits"
//         )
...
//   // **Fetch Related & Most Loved Meals**
...
//       // Define the expected structure of User with populated moodGoal
//       interface PopulatedUser {
//         moodGoal: { name: string } | null;
//         likes?: { recipe: string }[];
//       }
  
//       // Fetch user moodGoal and liked recipes
//       const user = await User.findById(userId)
//         .select("moodGoal likes")
//         .populate("moodGoal", "name")
//         .lean<PopulatedUser>();
  
//       if (!user || !user.moodGoal) {
//         return next(httpError(next, "MoodGoal not found or invalid", req, 404));
//       }
  
//       // Convert liked recipe IDs to a Set for quick lookup
//       const likedRecipes = new Set(user.likes?.map((like) => like.recipe.toString()) || []);
  
//       // Fetch related and most loved meals concurrently
//       const [relatedMeals, mostLovedMeals] = await Promise.all([
//         Recipe.find({ mood: user.moodGoal.name })
//           .sort({ rating: -1 })
//           .limit(2)
//           .select("_id imageUrl name prepTime rating mood")
//           .lean(),
...
//       // Add isFavorite: true/false based on liked recipes
//       const addFavoriteStatus = (meals: any[]) =>
//         meals.map((meal) => ({
//           ...meal,
//           isFavorite: likedRecipes.has(meal._id.toString()),
//         }));
...
//         relatedMeals: addFavoriteStatus(relatedMeals),
//         mostLovedMeals: addFavoriteStatus(mostLovedMeals),
//       });
//     } catch (error) {
//       next(httpError(next, error, req, 500));
//     }
...
//       const user = await User.findById(userId)
//         .select("moodGoal likes")
//         .populate<{ moodGoal: { name: string } }>("moodGoal", "name")
//         .lean();
  
//       if (!user?.moodGoal?.name) {
//         return next(httpError(next, "MoodGoal not found or invalid", req, 404));
//       }
  
//       const likedRecipes = new Set(user.likes?.map((like) => like.recipe.toString()) || []);
  
//       // Fetch related meals based on user's moodGoal
//       const relatedMeals = await Recipe.find({ mood: user.moodGoal.name })
//         .sort({ rating: -1 })
...
//       const RelatedMeals = relatedMeals.map((meal) => ({
//         ...meal,
//         isFavorite: likedRecipes.has(meal._id.toString()),
//       }));
  
//       httpResponse(req, res, 200, "Related meals fetched successfully", { RelatedMeals });
//     } catch (error) {
//       next(httpError(next, error, req, 500));
//     }
//   },
...
//   // **Fetch Mood Goals List**
//   // moodGoalList: async (req: Request, res: Response, next: NextFunction) => {
//   //   try {
//   //     const moodGoals = await MoodGoalModel.find().lean();
//   //     httpResponse(req, res, 200, "Mood goals fetched successfully", { moodGoals });
//   //   } catch (error) {
//   //     next(httpError(next, error, req, 500));
//   //   }
//   // },
...
//   favoriteMeals: async (req: Request, res: Response, next: NextFunction): Promise<void> => {
//     try {
//       const userId = req.user?.userId;
//       if (!userId) {
...
//       // Fetch user likes
//       const user = await User.findById(userId).select("likes").lean();
//       const likedRecipeIds = new Set(user?.likes?.map((like) => like.recipe.toString()) || []);
  
//       // Fetch all meals
//       const allMeals = await Recipe.find()
//         .select("imageUrl _id name prepTime rating")
//         .lean();
  
//       if (!allMeals || allMeals.length === 0) {
//         next(httpError(next, "No meals found", req, 404));
//         return;
//       }
  
//       // Add isFavorite field to each meal
//       const mealsWithFavorites = allMeals.map((meal) => ({
//         ...meal,
//         isFavorite: likedRecipeIds.has(meal._id.toString()), // True if liked, false otherwise
//       }));
  
//       httpResponse(req, res, 200, "Meals with favorite status fetched successfully", {
...
//   savedFavoriteMeal: async (req: Request, res: Response, next: NextFunction): Promise<void> => {
//     try {
//       const userId = req.user?.userId;
//       const { recipeId } = req.params;
...
//       if (!userId) {
//         return next(httpError(next, "Unauthorized. Please log in.", req, 401));
//       }
    
//       if (!recipeId || !mongoose.Types.ObjectId.isValid(recipeId)) {
//         return next(httpError(next, "Invalid Recipe ID provided.", req, 400));
//       }
    
//       if (typeof isFavorite !== "boolean") {
//         return next(httpError(next, "Invalid isFavorite value.. Must be true/false.", req, 400));
//       }
    
//       const recipeExists = await Recipe.exists({ _id: recipeId });
//       if (!recipeExists) {
//         return next(httpError(next, "Recipe not found.", req, 404));
//       }
...
//       const userLikes = Array.isArray(user.likes) ? user.likes : [];
//       const alreadyLiked = userLikes.some((like) => 
//         like.recipe?.toString() === recipeId
//       );
    
//       if (isFavorite && alreadyLiked) {
//         return httpResponse(req, res, 400, "You have already liked this recipe.", { isFavorite });
//       }
//       if (!isFavorite && !alreadyLiked) {
//         return httpResponse(req, res, 400, "You haven't liked this recipe yet.", { isFavorite});
//       }
...
//           .populate({ 
//             path: "likes.recipe", 
//             select: "imageUrl name prepTime rating -_id" 
//           })
//           .lean();
//       } else {
//         updatedUser = await User.findByIdAndUpdate(
...
//       if (!updatedUser) {
//         return next(httpError(next, "Failed to update user data.", req, 500));
//       }
...
//         res,
//         200,
//         `Recipe ${isFavorite ? "saved to" : "removed from"} favorites successfully`,
//         { 
//           // likedRecipes: Array.isArray(updatedUser.likes) ? updatedUser.likes : [],
//           isFavorite: isFavorite 
//         }
//       );
    
//     } catch (error) {
//       console.error("Error in savedFavoriteMeal:", error);
//       return next(httpError(next, "Something went wrong. Please try again later.", req, 500));
//     }
...
relatedMostLovedMeal: async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    httpResponse(req, res, 200, "Related and most loved meals fetched successfully", {
      relatedMeals: formatMeals(relatedMeals),
      mostLovedMeals: formatMeals(mostLovedMeals),
    });
  } catch (error) {
    next(httpError(next, error, req, 500));
  }
  ...
  favoriteMeals: async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const favoriteMealsList = await Recipe.aggregate([
  ...
  savedFavoriteMeal: async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const update = isFavorite
        ? { $addToSet: { likes: { recipe: recipeId, isFavorite: true } } }
        : { $pull: { likes: { recipe: recipeId } } };
