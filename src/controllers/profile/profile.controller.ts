...
export const getUserProfile = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const user = await User.findById(userId).select("-password -tokens -__v").populate('friends').populate('dietTypes')
      .populate('allergies').populate('moodGoal').populate('activityLevel').populate('likes.recipe').populate('saved')
...
/* This code snippet is defining a Mongoose schema for a user in a TypeScript application. Let's break
  ...
  try {
    ...
    if (userName === undefined && userEmail === undefined) {
      return httpResponse(
        ...
        res,
        400,
        "At least one field (userName or userEmail) must be provided"
      );
    }
