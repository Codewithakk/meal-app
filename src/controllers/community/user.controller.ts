...
export const getUserList = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        //     .skip(skip)
        //     .limit(limit)
        //     .select('userProfile userName _id firstName lastName')
        //     .lean();
...
export const addAndRemoveFriend = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const userObjectId = new mongoose.Types.ObjectId(friendId) as any;
        const isAlreadyFriend = user.friends.includes(userObjectId)
        if (isAlreadyFriend) {
            // Remove Firend
            const friendIndex = user.friends.findIndex((c: any) => { return c.toString() === userObjectId.toString() });
            if (friendIndex === -1) {
                return httpResponse(req, res, 404, "User not found");
            }

            const index = user.friends[friendIndex];

            // Remove friend by its index
            user.friends.splice(index as any, 1);
        } else {
            // Add Firend
