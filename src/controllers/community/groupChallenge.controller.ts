                    ...
                    pipeline: [
                        {
                            $match: {
                                ...
                                ...(regex && {
                                    $or: [{ firstName: { $regex: regex, $options: 'i' } }, { lastName: { $regex: regex, $options: 'i' } }]
                                })
                            }
                        },
                        {
...
export const getUserListByRank = async (
    userExist: { friends: mongoose.Types.ObjectId[] } | null,
                ...
                pipeline: [
                    {
                        $match: {
                            $expr: {
                                $and: [{ $eq: ['$user', '$$userId'] }, { $eq: ['$groupchallengeId', new mongoose.Types.ObjectId(groupchallengeId)] }]
                            }
                        }
                    },
                    {
                ...
                rank: {
                    $cond: [
                        { $gt: [{ $add: ['$totalLikes', '$totalDislikes'] }, 0] },
                        {
                            $multiply: [
                                {
                                    $divide: [{ $subtract: ['$totalLikes', '$totalDislikes'] }, { $add: ['$totalLikes', '$totalDislikes', 1] }]
                                },
                                100
                            ]
                        },
...
export const challengeWinnerUserAnnouncement = async (groupchallengeId: string, skip: number, limit: number, message: string) => {
    ...
    const userList = await getUserListByRank({ friends: [] }, joinedUserIds, groupchallengeId, skip, limit)
...
export const getJoinUserRateByPost = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const totalUsers = await User.aggregate([{ $match: { _id: { $in: joinedUserIds } } }, { $count: 'total' }])
        ...
        const userRanks = await getUserListByRank(
            userExist ? { friends: (userExist.friends || []).map((friend) => new mongoose.Types.ObjectId(friend)) } : null,
            joinedUserIds,
            groupchallengeId,
            skip,
            limit
