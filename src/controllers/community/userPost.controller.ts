...
export const createPost = async (req: Request, res: Response, next: NextFunction) => {
  let images = req.files ? (req.files as Express.Multer.File[]).map((file) => file.path) : [];
...
export const deletePostImage = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (imageList && imageList.length > 0) {
      const imageNames = imageList.map((image: string) => {
      ...
      if (imageNames.length > 0) {
        const imagesToDelete = post.images.filter((imageUrl: string) =>
          imageNames.some((imageName: string) => imageUrl.includes(imageName))
        );
...
export const viewPost = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    return next(httpError(next, res, error, 500));
  }
};
...
export const likePost = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (post.likes.includes(userObjectId)) {
      ...
      const likeIndex = post.likes.findIndex((c: any) => { return c.toString() === userObjectId.toString() });
      if (likeIndex === -1) {
      ...
      post.likes.splice(like as any, 1);
    } else {
      // Check if the user has already disliked the post
      if (post.dislikes.includes(userObjectId)) {
        // If the user has already disliked, remove the like
        const disLikeIndex = post.dislikes.findIndex((c: any) => { return c.toString() === userObjectId.toString() });
        if (disLikeIndex === 0) {
          const dislike = post.dislikes[disLikeIndex];

          // Remove the dislike by its index
          post.dislikes.splice(dislike as any, 1);
        }
      }
...
export const addComment = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    post.comments.push({
      ...
      createdAt: new Date(),
    } as any); // or as IComment, depending on what IComment expects
